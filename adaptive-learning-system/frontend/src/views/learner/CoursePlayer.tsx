...
/**
 * React view component representing the user interface dashboard or page for Course Player.
 */
...
export const CoursePlayer = () => {
  ...
  const hasQuizModules = modules.some(m => m.type === 'quiz');
  ...
  const isCurrentLocked = !isCurrentCompleted && currentAccess !== undefined && !currentAccess.canAccess && !isFirstModule;
  ...
  const handleQuizSubmit = () => {
    ...
    if (enrollment && currentModule.threshold !== undefined) {
      ...
      if (score < currentModule.threshold) {
        setTimeout(() => {
  ...
  const renderModuleContent = () => {
    ...
    if (isCurrentLocked && !isRemediationLocked) {
                ...
                {q.options.map((opt, oIdx) => {
                  ...
                  return (
                    <button key={oIdx} type="button" disabled={quizSubmitted}
                      ...
                      className={`w-full text-left px-4 py-3 rounded-xl border text-sm font-medium transition-all ${isCorrect ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-300 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400'
    ...
    if (currentModule.type === 'video' && currentModule.contentUrl) {
      return (
        <GlassContainer className="p-0 overflow-hidden">
          ...
          {!isCurrentCompleted && !isCurrentLocked && (
            <div className="p-6">
    ...
    if (currentModule.type === 'pdf' && currentModule.contentUrl) {
      return (
        <GlassContainer className="p-0 overflow-hidden relative">
          <PdfViewer
            key={currentModule.id}
            url={getFileUrl(currentModule.contentUrl)}
