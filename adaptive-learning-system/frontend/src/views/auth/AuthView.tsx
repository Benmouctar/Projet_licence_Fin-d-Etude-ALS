...
import { Mail, Lock, UserPlus, LogIn, User, ArrowLeft, CheckCircle } from 'lucide-react';
...
/**
 * React view component representing the user interface dashboard or page for Authentication View.
 */
...
export const AuthView = ({ initialMode = 'login' }: AuthViewProps) => {
  ...
  const { login, registerOnly } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        await registerOnly(name, email, password);
          ...
          <AnimatePresence>
            {registrationSuccess && (
              <motion.div
                ...
                className="mb-4 flex items-center gap-3 px-4 py-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20"
              >
                <CheckCircle className="w-4 h-4 text-emerald-500 shrink-0" />
                <div>
                  <p className="text-sm font-semibold text-emerald-700 dark:text-emerald-400">
                    Account created! Welcome to Co-Learn.
                  </p>
                  <p className="text-xs text-emerald-600 dark:text-emerald-500 mt-0.5">
                    Taking you to login in 2 seconds…
                  </p>
                </div>
              </motion.div>
            )}
