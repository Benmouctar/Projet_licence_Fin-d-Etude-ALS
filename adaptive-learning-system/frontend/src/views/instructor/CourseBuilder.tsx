...
/**
 * React view component representing the user interface dashboard or page for Course Builder.
 */
...
export const CourseBuilder = () => {
    ...
    createModule({ courseId: id, data: modulePayload }, {
      onSuccess: async (created) => {
        try {
          ...
          setNewContent('');
          setNewThreshold('50');
          setNewQuestions([{ question: '', options: ['', '', '', ''], correct: 0, difficultyLevel: 'MEDIUM' }]);
          setShowAdd(false);
        } catch {
  ...
  const handleReorder = async (index: number, direction: 'up' | 'down') => {
    ...
    if (swapIdx < 0 || swapIdx >= localModules.length) return;
    ...
    try {
      await Promise.all(
        reordered.map((mod, idx) =>
          ...
          moduleService.update(mod.id, { displayOrder: idx + 1 })
        )
      );
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    <button
                      ...
                      disabled={idx === localModules.length - 1}
                      className="p-0.5 text-slate-400 hover:text-emerald-600 disabled:opacity-20 transition-colors"
                      title="Move down"
                    >
