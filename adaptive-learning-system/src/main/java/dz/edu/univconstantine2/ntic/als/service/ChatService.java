...
import dz.edu.univconstantine2.ntic.als.service.ai.router.RagPipelineRouter;
...
/**
 * Service class containing business logic, validation rules, and transactional processing for Chat.
 */
@Service
...
public class ChatService {
    ...
    private final RagPipelineRouter ragPipelineRouter;
    ...
    public ChatResponseDTO processQuery(ChatQueryRequestDTO request, String userEmail) {
        ...
        RAGResult ragResult = ragPipelineRouter.retrieveContext(request.query(), request.moduleId());

        
        String aiResponse = ragPipelineRouter.generateResponse(
                request.query(),
                request.moduleId(),
                courseName,
                module.getTitle(),
        ...
        Message message = Message.builder()
                ...
                .ragPipelineUsed(ragPipelineRouter.isGeminiActive() ? "gemini" : "legacy")
                .build();
